...
class OrderableGroup extends React.Component {
  ...
  handleGroupItemUpdate(id, orderableChecked) {
    const orderGroup = this.props.groupDetails;
    let newCartGroup = [];

    const groupIsInCart = this.props.checkListSame(orderGroup, 1, 'Group');
    ...
    if (!orderableChecked) {
      ...
      const index = gCartList.findIndex((x => x.id === id.id));
  ...
  handleInputChange(details, event) {
    ...
    const cartGroup = new this.PatientGroup(details.id, details.text, groupCartList);
    if (this.state.cartGroup.length !== 0) {if (!checked) {groupCartList = [];}}
  ...
  render() {
    ...
    if (currentCheck[0] === false) {checkboxValue = false;} else if (orderGroup.value.length === currentCheck[1]) {
    ...
    let warning = 0;
    if (this.props.orderWarnings === 1) {warning = 1;} else if (this.props.orderWarnings === 0) {warning = 0;} else {
    ...
    // Handle multiple Orderables  in a group
      ...
      if (!this.state.open) {
        viewOrderables = null, duplicateStyle = 'notDuplicate';
      } else {
        viewOrderables = (
          orderGroup.value.map((orderable, index) => {
            ...
            if (0) {
              let orderableChecked;
              const mem = this.state.groupCartList.concat([]);
              const ind = mem.findIndex((x => x.id === orderable.id));
              if (mem[0] === []) {orderableChecked = false; } else if (ind === -1) {orderableChecked = false;} else {orderableChecked = true;}
            }
